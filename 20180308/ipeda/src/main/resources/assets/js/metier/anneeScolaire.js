...
function openModalUpdate(modalId, id) {
	...
	if(tr != false) {
		...
		container.find("#dateDebut").val(tr.find("td:eq(1)").text());
		container.find("#dateFin").val(tr.find("td:eq(2)").text());
...
function openModalDelete(modalId, id) {
	...
	if(tr != false) {
		...
		container.find("#message").html(message.replace("{0}", tr.find("td:eq(1)").text()) + "/" + tr.find("td:eq(2)").text());
		container.modal("show");
	}
}
